...
python3 -m fastchat.serve.controller --host 0.0.0.0 --port 11002
python3 -m fastchat.serve.register_worker --controller http://localhost:11002 --worker-name https://
python3 -m fastchat.serve.test_message --model vicuna-13b --controller http://localhost:11002
...
python3 -m fastchat.serve.gradio_web_server_multi --controller http://localhost:11002 --concurrency 10 --add-chatgpt --add-claude --add-palm --anony-only --elo ~/elo_results/elo_results.pkl --leaderboard-table-file ~/elo_results/leaderboard_table.csv --register ~/elo_results/register_oai_models.json --show-terms
